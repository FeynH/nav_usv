...
void geo_pos_conv::set_llh_nmea_degrees(double latd, double lond, double h)  //将接收到的经纬度转换成以弧度为单位的数据
{
  //double lat, lad, lod, lon;
  // 1234.56 -> 12'34.56 -> 12+ 34.56/60    //当接受的经纬度是度、分格式的时候
  ...
  m_lat=latd*M_PI/180;     //接受到的GPYBM报文中的经纬度是以度为单位的
  m_lon=lond*M_PI/180;
  m_h = h;
  // m_PLato=34.245488*M_PI/ 180.0;        //plane lat
	// m_PLo=108.982037*M_PI/ 180.0;
  conv_llh2xyz();
}

void geo_pos_conv::llh_to_xyz(double lat, double lon, double ele)  //直接将经纬度赋值给xyz--cartogragher
{
  // m_lat = lat * M_PI / 180;
  // m_lon = lon * M_PI / 180;
  // m_h = ele;
